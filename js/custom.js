...
if (popupForm) {
	let popupFormExit = popupForm.querySelector('.form-popup__exit');
	let popupFormContent = popupForm.querySelector('.form-popup__content');

	if (popupFormExit) {
		popupFormExit.addEventListener( 'click', (e) => {
			popupForm.classList.remove('_active');
		})
	}
	if (popupFormContent) {
		document.addEventListener( 'click', (e) => {
			let withinBoundaries = e.composedPath().includes(popupFormContent);
...
document.addEventListener("DOMContentLoaded", function () {
	...
	for (let i = 0; i < forms.length; i++) {
		...
		form.addEventListener("submit", function (e) {
			...
			if (error === 0) {
				//Это происходит если все поля заполнены
				//form.submit();
				form.classList.add('_heading-off');
				if (popupForm) {
